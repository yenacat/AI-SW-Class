...
class Sftp:
    ...
    def download(self, remote_path, target_local_path):
        ...
        Also, by default extracts the files to the specified target_local_path
        """

        try:
            print(
                f"Downloading from {self.hostname} as {self.username} [(remote path: {remote_path}); (target local path: {target_local_path})]"
            )
            ...
            # Download from the remote sftp server to local
...
if __name__ == "__main__":
    sftp_url = os.environ.get("SFTPOGO_URL")

    if not sftp_url:
        print("First, please set environment variable SFTPOGO_URL and try again.")
        exit(0)
    ...
    local_path = "C:/Users/UserK/Downloads/hollandlop.png"
    remote_path = "/hollandlop.png"
